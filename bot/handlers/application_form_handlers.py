...
async def send_application_form_info_handler(
...
async def cancel_fsm(callback: types.CallbackQuery, state: FSMContext) -> None:
    await(servises.cancel_fill_form_service(
        callback=callback,
        state=state,
...
async def restart_main_menu(
    ...
    try:
        kb = await servises.select_kb_service(session=session, id=callback.from_user.id)
    ...
    try:
        ...
        if (
            ...
            if (not one_day_gone and user.filled_form_count >= 3):
                rest_time = servises.count_rest_time_service(
...
async def upload_product_photo_one_fsm(
    ...
    url = await servises.convert_id_to_url_service(bot=bot, file_id=id)
...
async def skip_addittional_info_fsm(
    ...
    await servises.check_data_service(message=callback.message,
                                      state=state,
...
async def set_additional_data(message: types.Message, state: FSMContext):
    ...
    await servises.check_data_service(message=message,
                                      state=state,
                                      keyboard=kb_send_data)
...
async def send_data_to_google_sheets(
    ...
    try:
        ...
        if (
            ...
            return
        await callback.message.answer(
            text='Двнные добавлены в таблицу',
            reply_markup=kb
        )
